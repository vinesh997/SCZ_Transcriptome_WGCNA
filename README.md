...
**About the script:** The analysis is divided into two sections. 1- Partial correlation of hemogram data with the PANSS scores (symptom severity) and 2- WGCNA of transcriptome profile with the symptom severity. 
The phenodata information for partial correlation and WGCNA can be obtained on request. The scripts are as provided above, and the details of each script are mentioned below. 
...
For differential gene expression analysis and WGCNA, raw RNA-seq files were processed using the scripts (https://github.com/satyajeetkhare/RNASeq_HISAT2_Gencode_hg38). The fastq files were obtained from INDA-CA (INCARP000275 and  INCARP000309). The raw count matrix generated is used for differential gene expression analysis and Weighted gene coexpression analysis (WGCNA).
